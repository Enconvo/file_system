...
interface Options extends RequestOptions {
    ...
    edits: Array<{ oldTodoText: string, newTodoText: string }>;
...
export default async function main(request: Request): Promise<Response> {
    ...
    const validPath = await validatePath(options.todoFilePath);
    const edits = options.edits.map(edit => ({
        oldText: edit.oldTodoText,
        newText: edit.newTodoText
    }));
    const result = await applyFileEdits(validPath, edits, options.dryRun);
